...
DEFAULT_SERIES = ("VIXCLS", "DGS10", "BAA10Y", "FEDFUNDS")
SERIES_ALIASES = {
    "VIX": "VIXCLS",
    "10Y": "DGS10",
    "UST10Y": "DGS10",
...
class FredDataSource(DataSource):
    ...
    def fetch_historical(
        ...
        for series_id in self._resolve_series_ids(tickers):
            releases = self._fetch_releases(series_id)
            if releases.empty:
    ...
    def health_check(self) -> bool:
        try:
            releases = self._fetch_releases("FEDFUNDS")
    ...
    def _fetch_releases(self, series_id: str) -> pd.DataFrame:
        client = self._get_client()
        self._before_request(f"all releases for {series_id}")

        try:
            releases = client.get_series_all_releases(series_id)
        except Exception as exc:
