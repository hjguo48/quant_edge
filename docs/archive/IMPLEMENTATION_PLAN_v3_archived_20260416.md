...
**生成日期：** 2026-03-28
**项目当前状态：** S2 Stage 1 进行中 — V5 特征集 (21f, 含 macro interaction) IC=0.059, G3 Gate 4/6 PASS (IC t-test p=0.055), 20D fusion 无增益; 待完成: 2015起17窗口walk-forward + 1D/5D IC测试 + DAG改造 + Quintile统计; 灰度运行中; Phase 3 W22 完成, W24+ 暂停 (2026-04-16)
...
**基线（S1 Phase E）：** IC = 0.058, 11/11 正窗口, G3 Gate 4/6 PASS

---

#### 数据源审计 (2026-04-14 全面验证)

**已验证可用的端点 (实测)：**

| # | 端点 | 数据 | 入库状态 | 活跃 ticker 覆盖率 |
|---|------|------|---------|-------------------|
| 1 | FMP /stable/earnings | EPS surprise (actual vs estimated) | ✅ earnings_estimates | 100% (607/609) |
| 2 | FMP /stable/analyst-estimates | Forward consensus (EPS/Revenue avg/high/low) | ✅ analyst_estimates | 22% (134/609, API 限制) |
| 3 | FMP /stable/insider-trading/search | Form 4 逐笔交易 | ✅ insider_trades | 🔄 回填中 |
| 4 | FMP /stable/sec-filings-search/symbol | 8-K/10-K/10-Q filings | ✅ sec_filings | 🔄 回填中 |
| 5 | FMP /stable/price-target-summary | 分析师目标价汇总 | ⏳ 待特征工程接入 | — |
| 6 | FMP /stable/price-target-consensus | 目标价共识 | ⏳ 待特征工程接入 | — |
| 7 | FMP /stable/insider-trading/statistics | 季度内部人交易汇总 | ⏳ 待特征工程接入 | — |
| 8 | Polygon /stocks/v1/short-interest | 做空比率/天数 (双周) | ✅ short_interest | 100% (606/609) |
| 9 | Polygon /v2/reference/news | 新闻 + sentiment insights | ✅ news_sentiment | 12% (仅 2024.08+) |

**不可用的端点 (实测 404/403)：**
upgrades-downgrades (全系列), price-target (逐条), institutional-holder, 13f, rating, score, social-sentiment, stock-news, press-releases, stock-grade, esg-score

**需升级才可用：** earning-call-transcript (402 Restricted)

**数据完整性修复记录 (2026-04-14)：**
- SPY 价格: 补齐 2015-01-02 ~ 2016-04-17 (324 rows from FMP)
- Universe membership: 从 185 rows → 85,101 rows (729 tickers, 2015-2026 月度快照)
- Labels: 全部 4 horizon (1D/5D/20D/60D) 从 2015 重新生成, 1,709,980 rows each
- Fundamentals 2015-2017: 回填中
- Earnings: 扩展至 2015 (28,416 rows, 677 tickers)
- All backfills: 起始日期统一到 2015-01-01
...
| 序号 | 任务 | 交付物 | 验收标准 |
|------|------|--------|----------|
| S2.1.0 | 日频 PIT 数据可用性契约 | PIT knowledge_time 规则文档 + pipeline 验证 | ✅ 已完成 (docs/pit_daily_contract.md) |
| S2.1.1 | 日频标签体系 | 修改 `src/labels/forward_returns.py` 支持 open-to-open | ✅ 已完成 (1D/5D parquet, 从2015重新生成) |
| S2.1.2a | 新数据源入库 | FMP earnings/analyst/insider/SEC + Polygon short-interest/news | ✅/🔄 入库中 (6张新表) |
| S2.1.2b | 新数据源特征工程 | earnings_surprise, earnings_revision, short_ratio, insider_net_buy, sec_8k_count, news_sentiment_score, overnight_gap, volume_surge | ⏳ 待入库完成 |
| S2.1.2c | 全量 IC screening | 旧特征 + 新数据源特征一起筛选 | ⏳ 依赖 S2.1.2b |
| S2.1.2d | Walk-forward 重跑 (从2015, ~17窗口) | Ridge + 新特征, purge/embargo, 更多窗口 | ⏳ 依赖 S2.1.2c |
| S2.1.2e | G3 Gate 重测 | 17窗口 → t-test 检验力增强 | ⏳ 依赖 S2.1.2d |
| S2.1.3 | 多 Horizon 融合 | 1D + 5D + 20D + 60D 四 horizon Ridge 融合 | ⏳ 依赖 S2.1.2e |
| S2.1.4 | 日频换手控制 | 继承 S1 hysteresis + shrinkage + no-trade zone | ⏳ 仅在日频 > 周频时做 |
| S2.1.5a | Airflow DAG 改造 — 日频推理 | daily_signal_pipeline DAG | ⏳ 仅在日频 > 周频时做 |
| S2.1.5b | Airflow DAG 改造 — 新数据源自动更新 | 在 daily_data_pipeline 中增加 earnings/analyst/short-interest/insider/SEC filings 增量拉取 | ⏳ |
| S2.1.5c | Quintile 预期收益统计 | per-quintile bootstrap 统计, 前端 Expected Performance 按 quintile 分层展示 | ⏳ |
| S2.1.6 | 日频 vs 周频对比实验 | 17-window walk-forward 对比报告 | ⏳ 依赖 S2.1.3 |
...
- 日均换手 < 1.5%
- 高 VIX 期间不系统性崩溃
- walk-forward 全部通过 embargo 验证
- G3 Gate 6/6 PASS（更多窗口提升检验力）

**注意：** 日频 IC 通常远低于周频/月频（0.01-0.03 vs 0.05-0.10），不能直接和周频 IC 比较。日频评估以 ICIR + post-cost net excess 为主，非 raw IC。

**Stage 1 执行进度 (2026-04-16 最终)：**

| 步骤 | 状态 | 详情 |
|------|------|------|
| S2.1.0 PIT 契约 | ✅ | docs/pit_daily_contract.md |
| S2.1.1 日频标签 | ✅ | 1D/5D/20D/60D 从 2015 生成, 1.71M rows each |
| S2.1.2a 数据入库 | ✅ | 全部完成: earnings(677t), analyst(632t live-only), short-interest(696t), insider(722t), SEC(643t), news(86t 2024+) |
| S2.1.2a 数据修复 | ✅ | SPY 2016-04→2026补齐(Polygon 10yr限制), Universe 重建, Fundamentals 2015补齐, Labels 重新生成 |
| S2.1.2b 特征工程 | ✅ | Batch 1+2: 13新特征(PEAD×2, reversal×3, beta, above_20dma, breadth×3, macro interaction×3), 总210候选 |
| S2.1.2c IC screening | ✅ | V5: 210候选→18 passed, 21 retained; **注意: IC screening 默认用 5D labels (HORIZON_DAYS=5), 60D true screening 进行中** |
| S2.1.2d Walk-forward 13窗口 | ✅ | V5 no-analyst(19f) 60D Ridge: **IC=0.062, t=2.17, p=0.025, 10/13正** |
| S2.1.2e G3 Gate | ✅ | V5 no-analyst 13窗口: **6/6 PASS** (SPA 修复为 vs zero-IC null) |
| S2.1.3 多Horizon测试 | ✅ | 1D IC=0.017(t=1.10, 不稳定), 5D IC=0.015(t=2.03, 10/13正), 20D IC=0.024(弱), 5D+60D融合无增益 |
| S2.1.4 日频换手控制 | 跳过 | 1D/5D post-cost 未超越 60D, 不切换日频 |
| S2.1.5a DAG日频推理 | 跳过 | 同上 |
| S2.1.5b DAG新数据源更新 | ✅ | fetch_alternative_data task 加入 daily_data_pipeline DAG, 增量模式 |
| S2.1.5c Quintile统计 | ✅ | quintile_stats_v5.json 生成 (Q1=34.2% ann excess, Q5=-5.3%) |
| S2.1.6 日频vs周频对比 | ✅ | 结论: 日频无增益, 5D IC=0.015 但 post-cost 不优于 60D weekly |

**IC 提升完整路径:**

| 版本 | 特征数 | IC | t-stat | p | G3 Gate | 关键变化 |
|------|--------|-----|--------|---|---------|---------|
| v3 baseline | 15 | 0.037 | 1.36 | 0.10 | 4/6 | sign_cons≥0.6, 11w |
| SA-v2 | 25 | 0.054 | 1.75 | 0.055 | 4/6 | sign_cons≥0.5, 11w |
| V4 | 20 | 0.059 | 1.75 | 0.055 | 4/6 | +macro interaction, 11w |
| V5 13w | 21 | 0.056 | 1.93 | 0.039 | 5/6 | 扩展到 13 窗口 |
| **V5 no-analyst** | **19** | **0.062** | **2.17** | **0.025** | **6/6** | **去掉 analyst_coverage, W10翻正** |

**Analyst ablation 关键发现:**
- analyst_coverage (Ridge #1 特征, IC=0.031) 实际拖累了模型
- ~75% tickers 在 analyst_estimates 中为 NaN (FMP 只有 forward-looking)
- 去掉后 W10 从 -0.042 翻正到 +0.031, W9 从 +0.061 提升到 +0.086
- is_missing_analyst_coverage 编码了 data-vendor coverage bias 而非真实信号

**弱窗口诊断 (Codex):**
- W5: val IC=+0.145 → test IC=-0.116, regime break 不可预测
- W6: val IC=-0.101 → test IC=-0.061, val 已负可 gating
- W11: val IC=-0.041 → test IC=-0.049, val 已负可 gating
- 仅剩 3 个负窗口 (W5/W6/W11), W10 已翻正

**5D 互补发现:**
- 5D IC=0.015, t=2.03, 10/13 正窗口
- 5D 在 W10(+0.008) 和 W11(+0.001) 为正 (60D 为负)
- 但 5D+60D 固定权重融合拉低 IC, 不采用
- 5D 可作为 validation-overlay 或 regime-aware 择时信号, 留待 Stage 3

**发现的技术问题:**
- IC screening 默认 HORIZON_DAYS=5, 之前所有 "60D screening" 实际用 5D labels
- 60D true screening (--horizon 60) 正在运行, 结果待比较
- G3 Gate IC t-test 使用 one-sided test (alternative="greater"), p=0.025 是 one-sided
- SPA benchmark 已修复: Ridge vs zero-IC null (不再是 Ridge vs 融合)
...
#### Stage 5：模型生命周期管理 — 自动重训 + 版本管理 + 漂移触发

**动机：** 模型上线后如果不定期重训，会逐渐过时（市场 regime 变化、新数据积累）。S1 Phase C 建了 PSI/CUSUM 监控，但只是告警不是触发器。需要完整的模型生命周期管理。

| 序号 | 任务 | 交付物 | 验收标准 |
|------|------|--------|----------|
| S2.5.1 | 定期自动重训 DAG | Airflow DAG: 每月第一个周末自动重训 Ridge，走完 walk-forward + IC screening + G3 Gate | 全自动无人干预；G3 Gate PASS 才替换 bundle，FAIL 则保留旧模型 + 告警 |
| S2.5.2 | 模型版本管理 | MLflow 注册新版模型，保留历史版本；bundle 带版本号 (v1/v2/v3...) | 可回滚到任意历史版本；每个版本有完整的 IC/Sharpe/G3 Gate 记录 |
| S2.5.3 | Champion/Challenger 机制 | 新模型先作为 Challenger 并行出信号，和 Champion 对比 1-2 周 | Challenger IC > Champion 才自动切换；切换后旧 Champion 降级为 Challenger 继续监控 |
| S2.5.4 | 漂移触发重训 | PSI > 0.25 连续 2 周 或 CUSUM 检测到变点 → 自动触发紧急重训 | 不等月度周期，漂移严重时立即重训 |
| S2.5.5 | 重训后自动更新前端数据 | G3 Gate 报告 + quintile 统计 + bundle 自动更新 | Expected Performance 等前端展示数据跟随模型更新 |
| S2.5.6 | 重训审计日志 | 每次重训记录：触发原因、新旧 IC 对比、G3 Gate 结果、是否替换 | 完整审计链，可追溯任何模型变更 |

**Stage 5 门控：**
- 自动重训 DAG 连续 3 个月稳定运行无故障
- Champion/Challenger 切换逻辑经过至少 2 次实际切换验证
- 漂移触发在模拟环境中测试通过

**依赖关系：**
- S2.5.1 依赖 S2.1.5（Airflow DAG 改造完成）
- S2.5.2 依赖 MLflow（已有，Day 1 就在用）
- S2.5.3 依赖 S2.5.1 + S2.5.2
- S2.5.4 依赖 S1 Phase C（PSI/CUSUM 已部署）+ S2.5.1

**现有基础：**
- ✅ PSI/CUSUM 监控已部署（S1 Phase C）— 只需加触发逻辑
- ✅ MLflow 实验跟踪（Day 1）— 只需加模型注册
- ✅ G3 Gate 脚本（已有）— 只需嵌入 DAG
- ❌ 自动重训 DAG — 需要新建
- ❌ Champion/Challenger — 需要新建
- ❌ 版本化 bundle — 需要改造
...
| Stage 5 MLOps | 模型稳定性 | 自动重训 + 版本管理 + 漂移触发 | |
