...
def collect_feature_observations(
    ...
    *,
    ...
    merged["is_missing"] = merged["is_missing_observed"].fillna(merged["is_missing"]).astype(bool)
    return (
...
def collect_pipeline_missing_observations(
    *,
    ...
    if not sampled_tickers or not sampled_trade_dates or not feature_names:
    ...
    for trade_day in sampled_trade_dates:
        ...
        if frame.empty:
            continue
        flag_rows = frame.loc[
            frame["feature_name"].astype(str).str.startswith(MISSING_FLAG_PREFIX),
            ["ticker", "trade_date", "feature_name", "feature_value"],
        ].copy()
        if flag_rows.empty:
            continue
        flag_rows["feature_name"] = flag_rows["feature_name"].astype(str).str.removeprefix(MISSING_FLAG_PREFIX)
        flag_rows = flag_rows.loc[flag_rows["feature_name"].isin(feature_name_set)].copy()
        if flag_rows.empty:
            continue
        flag_rows["trade_date"] = pd.to_datetime(flag_rows["trade_date"]).dt.date
        flag_rows["is_missing"] = pd.to_numeric(flag_rows["feature_value"], errors="coerce").fillna(1.0) >= 0.5
        frames.append(flag_rows[["ticker", "trade_date", "feature_name", "is_missing"]])
